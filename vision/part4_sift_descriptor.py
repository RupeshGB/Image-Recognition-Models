...
def get_gradient_histogram_vec_from_patch(
    ...
    # Define constants for the number of cells and orientations
    num_cells_x, num_cells_y = 4, 4
    num_orientations = 8

    # Calculate the cell size in pixels
    cell_size = 16 // num_cells_x

    histograms = []

    bins = np.linspace(-np.pi, np.pi, num_orientations + 1) - 1e-5
    # Iterate over each cell in the 4x4 grid
    for i in range(num_cells_x):
        for j in range(num_cells_y):
            # Extract the current cell's region from the patch
            cell_magnitudes = window_magnitudes[i * cell_size:(i + 1) * cell_size, j * cell_size:(j + 1) * cell_size]
            cell_orientations = window_orientations[i * cell_size:(i + 1) * cell_size, j * cell_size:(j + 1) * cell_size]

            # Calculate the weighted histogram for the current cell
            # histogram, _ = np.histogram(cell_orientations, bins=num_orientations, range=(-np.pi, np.pi), weights=cell_magnitudes)
            histogram, _ = np.histogram(cell_orientations, bins=bins, weights=cell_magnitudes)




            histograms.append(histogram)
            # Append the histogram values to the weighted gradient histogram vector
            #start_idx = (i * num_cells_x + j) * num_orientations
            #end_idx = start_idx + num_orientations
            #wgh[start_idx:end_idx] = histogram.reshape(-1, 1)
    wgh = np.array(histograms)
...
def get_feat_vec(
    ...
    window_magnitudes = magnitudes[r - feature_width // 2+1:r + feature_width // 2 + 1, (c - feature_width // 2+1):c + feature_width // 2+1]
    window_orientations = orientations[(r - feature_width // 2+1):(r + feature_width // 2+1), (c - feature_width // 2+1):c + feature_width // 2+1]

    # Get the weighted gradient histogram vector for the window
    fv = get_gradient_histogram_vec_from_patch(window_magnitudes, window_orientations)

    # Reshape the weighted gradient histogram vector and normalize it to unit length
    # fv[:, 0] = wgh.reshape(-1)
    norm = np.linalg.norm(fv)
    if norm > 0:
        fv /= norm

    # Apply square-root SIFT
    fv = np.sqrt(fv)
...
def get_SIFT_descriptors(
    ...
    ###########################################################################
    m, n = image_bw.shape
    magnitudes, orientations = get_magnitudes_and_orientations(*compute_image_gradients(image_bw))

    # Compute SIFT descriptors for each interest point
    fvs = []
    for x, y in zip(X, Y):
        # if x >= feature_width // 2 and x < n - feature_width // 2 and y >= feature_width // 2 and y < m - feature_width // 2:
        fv = get_feat_vec(x, y, magnitudes, orientations, feature_width).flatten()
        fvs.append(fv)

    # Convert the list of feature vectors to a numpy array
    fvs_array = np.array(fvs)
